export function parseCSV(csvText: string): any[] {
  ...
  return cleanLines.slice(1).map((line) => {
    ...
    headers.forEach((header: string, index: number) => {
      ...
      else if (header === 'urlkelolaakun' || header === 'linkkelolaakun' || header.includes('kelolaakun') || header === 'linkakun' || header === 'urlakun' || header.includes('kelolajurusan')) keyName = 'urlKelolaAkun';
